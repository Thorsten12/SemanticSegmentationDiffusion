...
class PH2ContourDataset(Dataset):
    ...
    def _contour_points(self, mask_binary: np.ndarray) -> np.ndarray:
        ...
        if not contours:
            raise ValueError("empty mask: no contours found")
    ...
    def __getitem__(self, idx: int):
        ...
        # --- NEU: Guard gegen leere Masken (auch beim Original-Sample!) ---
        if mask_binary.sum() == 0:
            # Kaputtes/leeres GT-Sample -> auf ein anderes Sample ausweichen,
            # statt in cv2.findContours/max() zu crashen.
