...
class GaussianDiffusion:
    ...
    def _min_snr_weight(self, t, gamma):
        """Min-SNR-gamma loss weighting (Hang et al., 2023).

        snr(t) = alpha_bar_t / (1 - alpha_bar_t)
        weight(t) = min(snr(t), gamma) / snr(t)

        Clamps the effective weight to <= 1 at low noise (high SNR) instead
        of letting easy/low-noise steps dominate the reconstruction loss.
        """
    ...
    def training_losses(self, predicted_noise, noise, x_t, x0, t, masks=None,
        ...
        if snr_gamma is not None:
            snr_w = self._min_snr_weight(t, snr_gamma).unsqueeze(-1)   # [B, 1] -> broadcast über N
            sq_err = sq_err * snr_w
