...
class Config:
    ...
    pvt_pretrained_path: str = "pretrained_pth/pvt_v2_b2.pth"

    vmamba_checkpoint: str | None = None
    ...
    # ----- LR schedule -----
    # scheduler="none" behaelt die alte (konstante) LR bei; "cosine" aktiviert
    # linearen Warmup + Cosine-Decay bis min_lr.
    #
    # WICHTIG: from_args() unten kopiert nur Felder, die tatsaechlich in dieser
    # Dataclass existieren (siehe `for f in fields(cls)`). Ohne diese drei
    # Felder hier werden --scheduler/--warmup_epochs/--min_lr auf der CLI
    # STILLSCHWEIGEND ignoriert -- kein Fehler, die LR bleibt einfach konstant
    # bei cfg.lr, egal was man uebergibt. Das war der Bug.
    scheduler: str = "none"
    warmup_epochs: int = 0
    min_lr: float = 0.0

    # ----- bookkeeping -----
    out_dir: str = "baseline_models/runs/baseline"
    seed: int = 0
    device: str = "cuda"
    log_every: int = 20
    eval_every: int = 20
