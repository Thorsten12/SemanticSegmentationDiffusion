...
class LoRAMultiheadAttention(nn.Module):
    def __init__(self, mha: nn.MultiheadAttention, r: int = 4):
        ...
        d = mha.embed_dim
        in_dim = mha.in_proj_weight.shape[1]
        self.A = nn.Parameter(torch.randn(3, r, in_dim) / math.sqrt(in_dim))
        self.B = nn.Parameter(torch.zeros(3, d, r))
 
    def forward(self, query, key, value, need_weights=False, attn_mask=None, **kwargs):
        delta = torch.einsum('nrk,ndr->ndk', self.A, self.B)
        delta = delta.reshape(3 * delta.shape[1], delta.shape[2])
        weight = self.mha.in_proj_weight + delta
        ...
        out, attn_w = F.multi_head_attention_forward(
            query, key, value,
            self.mha.embed_dim, self.mha.num_heads,
            weight, self.mha.in_proj_bias,
            self.mha.bias_k, self.mha.bias_v,
            self.mha.add_zero_attn, self.mha.dropout,
            self.mha.out_proj.weight, self.mha.out_proj.bias,
            training=self.mha.training, need_weights=need_weights, attn_mask=attn_mask)
...
def inject_lora(model, r=4):
    for name, module in list(model.named_modules()):
        if isinstance(module, nn.MultiheadAttention):
            ...
            setattr(parent, child_name, LoRAMultiheadAttention(module, r=r))
    return model
...
if __name__ == "__main__":
    ...
    backbone = inject_lora(backbone, r=4)
    print(backbone)
