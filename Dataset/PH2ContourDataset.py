...
class PH2ContourDataset(Dataset):
    def __init__(self, base_path, n_punkte=200, img_size=(256, 256)):
        """
        base_path : Pfad zu "PH2 Dataset images"
        ...
        print(f"PH2 Datensatz: {len(self.samples)} Bilder gefunden.")

        # ----------------------------------------------------------
        #  2. Alles einmalig in RAM laden (PH2: ~200 Bilder ≈ 50 MB)
        # ----------------------------------------------------------
        H, W = img_size
    ...
    def __getitem__(self, idx):
        ...
        # Aus Cache laden – kein Disk-I/O mehr
        ...
        if touches_edge:
            # Fallback: Original-Daten aus Cache (kein Disk-I/O, kein findContours)
            img         = Image.fromarray(self.cache[idx]['img'])
