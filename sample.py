...
def load_checkpoint(ckpt_path, cfg, device):
    ...
    if saved:
        for k in ("encoder", "backbone", "cond_channels", "stem_dim", "n_points", "hidden_dim",
                  ...
                  "top_k_scales",
                  "use_global_attn", "global_attn_levels", "global_attn_grid",
                  "global_attn_position", "global_attn_gate_schedule",
                  "global_attn_gate_min", "global_attn_gate_max", "local_drop_coarsest"):
            if k in saved:
    ...
    denoiser = ContourDenoiser(
        ...
        use_global_attn=getattr(cfg, "use_global_attn", False),
        global_attn_levels=getattr(cfg, "global_attn_levels", 2),
        global_attn_grid=getattr(cfg, "global_attn_grid", 14),
        global_attn_position=getattr(cfg, "global_attn_position", "pre"),
        global_attn_gate_schedule=getattr(cfg, "global_attn_gate_schedule", "linear"),
        global_attn_gate_min=getattr(cfg, "global_attn_gate_min", 0.20),
        global_attn_gate_max=getattr(cfg, "global_attn_gate_max", 1.00),
        local_drop_coarsest=getattr(cfg, "local_drop_coarsest", 0)
    ).to(device)
